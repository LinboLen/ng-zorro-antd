...
describe('tree-view based on nzLevelAccessor', () => {
  ...
  describe('tree view basic', () => {
    let fixture: ComponentFixture<TriTestTreeViewBasicWithLevelAccessorComponent>;
    let testComponent: TriTestTreeViewBasicWithLevelAccessorComponent;
    let nativeElement: Element;

    beforeEach(async () => {
      fixture = TestBed.createComponent(TriTestTreeViewBasicWithLevelAccessorComponent);
    ...
    it('should disabled node can not be selected but can be expanded', async () => {
      ...
      expect((disabledNode.componentInstance as TriTreeNodeComponent<FlatNode>).data.name).toBe('parent 1-1');
  ...
  describe('flattener', () => {
    let flattener: TriTreeFlattener<TreeNode, FlatNode>;
    ...
    it('should flattenNodes method transform nested nodes to flatten nodes', () => {
      flattener = new TriTreeFlattener(
        transformer,
        node => node.level,
  ...
  describe('padding', () => {
    ...
    it('should nzTreeNodePadding without value work', () => {
      ...
      nodes.forEach(node => {
        expect(window.getComputedStyle(node.nativeElement).paddingLeft).toBe(
          `${(node.componentInstance as TriTreeNodeComponent<FlatNode>).data.level * defaultIndent}px`
        );
      });
    });
    ...
    it('should nzTreeNodePaddingIndent work', async () => {
      ...
      nodes.forEach(node => {
        expect(window.getComputedStyle(node.nativeElement).paddingLeft).toBe(
          `${(node.componentInstance as TriTreeNodeComponent<FlatNode>).data.level * indent}px`
        );
      });
    });
  });
