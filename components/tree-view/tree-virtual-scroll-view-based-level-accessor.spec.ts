...
describe('virtual scroll based nzLevelAccessor', () => {
  ...
  describe('basic', () => {
    let fixture: ComponentFixture<TriTestTreeViewVirtualScrollWithLevelAccessorComponent>;
    let testComponent: TriTestTreeViewVirtualScrollWithLevelAccessorComponent;
    let tree: TriTreeVirtualScrollViewComponent<FlatNode>;
    let viewport: CdkVirtualScrollViewport;

    beforeEach(() => {
      fixture = TestBed.createComponent(TriTestTreeViewVirtualScrollWithLevelAccessorComponent);
  ...
  describe('padding', () => {
    ...
    it('should nzTreeNodePadding with value work', async () => {
      ...
      expect(window.getComputedStyle(firstNode.nativeElement).paddingLeft).toBe('0px');
      const firstNode_paddingDir = firstNode.injector.get(TriTreeNodePaddingDirective);
...
export class TriTestTreeViewVirtualScrollWithLevelAccessorComponent implements OnInit {
  ...
  readonly trackBy = signal<TrackByFunction<FlatNode>>((_index: number, value: FlatNode): TriSafeAny => value);
