...
import { DRAWER_ANIMATE_DURATION, TriDrawerComponent } from './drawer.component';
...
describe('NzDrawerComponent', () => {
  describe('default', () => {
    ...
    describe('placement', () => {
      const placements: TriDrawerPlacement[] = ['left', 'right', 'top', 'bottom'];
      placements.forEach(placement => {
        it(`should nzPlacement work (${placement})`, () => {
          component.placement.set(placement as TriDrawerPlacement);
          component.open();
          fixture.detectChanges();
...
describe('NzDrawerService', () => {
  ...
  beforeEach(() => {
    ...
    drawerService = TestBed.inject(TriDrawerService);
    fixture.detectChanges();
  });
  ...
  it('should not replace an input signal with nzData', () => {
    const drawerRef = drawerService.create({
      content: TriDrawerContentWithInputSignalComponent,
      data: { text: 'Drawer data' }
    });
    ...
    expect(contentComponent?.data.text).toBe('Drawer data');
  });
...
class TriDrawerContentWithInputSignalComponent {
  readonly data = inject<{ text: string }>(TRI_DRAWER_DATA);
