...
import { ColSize, TriColDirective } from './col.directive';
import { TriGridModule } from './grid.module';
import { TriAlign, Gutter, TriJustify, TriRowDirective } from './row.directive';
  ...
  describe('col', () => {
    ...
    it('should apply className according to property', () => {
      const propertySizeMatch = (name: keyof TestColComponent, count: number): boolean => {
        (component[name] as WritableSignal<TriSafeAny>).set(count);
        fixture.detectChanges();
...
@Component({
  ...
  template: `
    <div tri-row [gutter]="gutter()" [justify]="justify()" [align]="align()" [wrap]="wrap()">
      <div tri-col></div>
    </div>
  `
})
export class TestGridComponent {
  ...
  justify = signal<TriJustify | Partial<ResponsiveLike<TriJustify>> | null>(null);
  align = signal<TriAlign | Partial<ResponsiveLike<TriAlign>> | null>(null);
