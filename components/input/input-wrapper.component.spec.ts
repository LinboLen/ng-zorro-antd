...
@Component({
  imports: [TriInputModule, FormField],
  template: `
    <tri-input-wrapper allowClear (clear)="onClear()">
      <input tri-input [formField]="form.name" />
    </tri-input-wrapper>
  `
})
...
@Component({
  imports: [TriInputModule, ReactiveFormsModule],
  template: `
    <tri-input-wrapper showCount [count]="countConfig">
      <input tri-input type="number" [formControl]="control" />
    </tri-input-wrapper>
  `
})
class InputCountNumberControlTestComponent {
  ...
  readonly countConfig: TriCountConfig = {
    max: 10,
...
export class InputWithCountTestComponent {
  ...
  readonly countConfig = signal<TriCountConfig>({});
