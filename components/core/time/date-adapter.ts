...
import { TRI_DATE_CONFIG, TRI_DATE_CONFIG_DEFAULT, TRI_DATE_LOCALE, TriDateConfig } from './date-config';
 ...
 */
export const TRI_DATE_ADAPTER = new InjectionToken<Type<TriDateAdapter<unknown>>>(
  typeof ngDevMode !== 'undefined' && ngDevMode ? 'nz-date-adapter-type' : ''
);

/** Configuration for a date adapter provider. */
export interface TriDateAdapterConfig<TLocale = unknown> extends TriDateConfig {
 ...
 */
export abstract class TriDateAdapter<TDate, TLocale = unknown> {
  /** The current locale. */
  ...
  abstract format(date: TDate, displayFormat: TriSafeAny): string;

  /** Parses a value into a date. */
  abstract parse(value: TriSafeAny, parseFormat: TriSafeAny): TDate | null;

  // --- Validation ---

  /** Checks whether the given object is a date instance. */
  abstract isDateInstance(obj: TriSafeAny): boolean;
   ...
   */
  deserialize(value: TriSafeAny): TDate | null {
  ...
  parseTime(_value: TriSafeAny, _parseFormat?: TriSafeAny): TDate | null {
    throw new Error(NOT_IMPLEMENTED);
  }
