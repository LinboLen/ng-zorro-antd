...
export interface TriNativeDateAdapterConfig extends TriDateAdapterConfig<string> {
 ...
 */
@Injectable()
export class NativeDateAdapter extends TriDateAdapter<Date, string> {
  private readonly dateLocale = inject(TRI_DATE_LOCALE, { optional: true });
  private readonly dateConfig = inject(TRI_DATE_CONFIG, { optional: true });
  ...
  getFirstDayOfWeek(): number {
    ...
    if (typeof Intl !== 'undefined' && (Intl as TriSafeAny).Locale) {
      const locale = new (Intl as TriSafeAny).Locale(this.locale) as {
  ...
  parse(value: TriSafeAny, parseFormat: TriSafeAny): Date | null {
  ...
  format(date: Date, displayFormat: TriSafeAny): string {
  ...
  override deserialize(value: TriSafeAny): Date | null {
    if (typeof value === 'string') {
      if (!value) {
        return null;
      }
  ...
  isDateInstance(obj: TriSafeAny): boolean {
  ...
  override parseTime(userValue: TriSafeAny, parseFormat?: TriSafeAny): Date | null {
