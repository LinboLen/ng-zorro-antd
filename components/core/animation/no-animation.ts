...
export const TRI_NO_ANIMATION_CLASS = 'nz-animate-disabled';

@Directive({
  selector: '[triNoAnimation]',
  exportAs: 'triNoAnimation',
  host: {
    [`[class.${TRI_NO_ANIMATION_CLASS}]`]: `nzNoAnimation() || animationType === 'NoopAnimations'`
  }
})
export class TriNoAnimationDirective {
  readonly animationType = inject(ANIMATION_MODULE_TYPE, { optional: true });
  readonly noAnimation = input(false, { transform: booleanAttribute });
}
...
export function withAnimationCheck(classNameGetter: () => string): Signal<string> {
  ...
  return animationType === 'NoopAnimations' ? signal(TRI_NO_ANIMATION_CLASS) : computed(classNameGetter);
}
