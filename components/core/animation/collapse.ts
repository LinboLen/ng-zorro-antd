...
import { isAnimationEnabled, TriNoAnimationDirective } from './no-animation';
...
export class TriAnimationCollapseDirective {
...
@Injectable()
export class TriAnimationTreeCollapseService {
  firstRender = true;
  virtualScroll = false;
...
export class TriAnimationTreeCollapseDirective {
  private readonly treeCollapseService = inject(TriAnimationTreeCollapseService, { optional: true });
  private readonly noAnimation = inject(TriNoAnimationDirective, { optional: true, host: true });
  // should disable animation in virtual scrolling
  private readonly animationEnabled = isAnimationEnabled(
    () => !this.noAnimation?.noAnimation() && !(this.treeCollapseService?.virtualScroll ?? false)
  );
