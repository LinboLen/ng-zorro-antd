...
describe('NzTrMeasureComponent', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [{ provide: TriResizeObserver, useValue: { observe: () => resizeEntries$ } }]
    });
...
@Component({
  template: `
    <table>
      <tbody>
        <tr tri-table-measure-row [listOfMeasureColumn]="['col']" (listOfAutoWidth)="onAutoWidth($event)"></tr>
      </tbody>
    </table>
  `,
  imports: [TriTrMeasureComponent]
})
class TestHostComponent {
  readonly measureComponent = viewChild.required(TriTrMeasureComponent);
  widths: number[] = [];
