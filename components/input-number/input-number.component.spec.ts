...
import { TriInputNumberComponent, TriInputNumberStepEvent } from './input-number.component';
...
describe('input-number', () => {
  ...
  it('should be focus / blur work', async () => {
    ...
    component.inputNumber()._focus({ cursor: 'start' });
    expect(input.selectionStart).toBe(0);
    expect(input.selectionEnd).toBe(0);
    component.inputNumber()._focus({ cursor: 'end' });
    ...
    component.inputNumber()._focus({ cursor: 'all' });
    expect(input.selectionStart).toBe(0);
    expect(input.selectionEnd).toBe(String(component.value).length);
  });
...
class InputNumberTestComponent {
  ...
  onStep(_event: TriInputNumberStepEvent): void {}
}
...
@Component({
  imports: [TriInputNumberModule],
  selector: 'tri-test-input-number-final-size',
