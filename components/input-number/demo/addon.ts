  ...
  template: `
    ...
    <tri-input-number [(ngModel)]="value">
      <tri-select inputAddonBefore ngModel="add" [style.width.px]="60">
        <tri-option label="+" value="add" />
        <tri-option label="-" value="minus" />
      </tri-select>
      <tri-select inputAddonAfter ngModel="USD" [style.width.px]="60">
        <tri-option value="USD" label="$" />
        <tri-option value="EUR" label="€" />
        <tri-option value="GBP" label="£" />
        <tri-option value="CNY" label="¥" />
      </tri-select>
    </tri-input-number>
