...
export class TriUploadBtnComponent implements OnInit {
  ...
  private upload(file: TriUploadFile, fileList: TriUploadFile[]): void {
    ...
    const successBeforeLoadHook = (processedFile: TriUploadFile | boolean | Blob | File): void => {
      const processedFileType = Object.prototype.toString.call(processedFile);
      if (
        typeof processedFile !== 'boolean' &&
        (processedFileType === '[object File]' || processedFileType === '[object Blob]')
      ) {
        (processedFile as TriUploadFile).uid = file.uid; // we are sure that the file has already an uid, now nzBeforeUpload is used to transform the file, the transform file needs to have the same uid as the original file
        this.post(file, processedFile as TriUploadFile);
      } else if (processedFile) {
        this.post(file);
      }
    };
    const errorBeforeLoadHook = (error: TriSafeAny): void => {
  ...
  private post(file: TriUploadFile, processedFile?: string | Blob | File | TriUploadFile): void {
    if (this.destroyed) {
      return;
    }
    let process$: Observable<string | Blob | File | TriUploadFile> = of(processedFile || file);
